...
The manuscript introduces **Evidence-Complete Transition Closure (ECTC)**, a protocol semantics in which a state transition is not terminal merely because a validator accepted it. A Capsule must reach a *Closed* outcome carrying a ClosureProof and successor StateSeal, a *Conflict* outcome carrying a recorded, auditable ConflictEvidence artifact, or an *Abandoned* outcome carrying an AbandonProof. The lifecycle binds predecessor state, transition claim, witness obligation, and successor state while preserving incompatible and abandoned claims as recorded, auditable artifacts. The implementation connects this lifecycle to leaderless dissemination, durable SQLite WAL recovery, Ed25519 signatures, weighted membership and settlement, commit–reveal randomness, Reed–Solomon availability, deterministic execution, onboarding, and a hash-chained event log.
