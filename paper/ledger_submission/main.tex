...
\author{Adnan Omar Awad Allemon,\thanks{A. O. A. Allemon is an independent researcher.}}
\pagestyle{pagemain}
...
The availability layer has working erasure coding, sampling, and repair paths, but the current evidence is local. A public deployment would need independent providers, retention and pruning policies, authenticated sampling schedules, withholding tests, repair under correlated failures, and a relation between provider stake and availability evidence. The deterministic execution kernel is not a general smart-contract VM and should not be presented as a replacement for Ethereum or Sui execution environments.
...
The source code, benchmark drivers, formal model, generated figures, tabular data, JSON artifacts, and checksums are available in the companion repository linked in the final manuscript version. The repository is intended to be public and excludes private keys, credentials, and local virtual environments. Because the current results were produced in local emulation, the artifact explicitly labels them as such.
